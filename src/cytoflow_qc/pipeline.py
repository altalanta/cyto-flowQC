...
from cytoflow_qc.io import load_dataframe, read_manifest, save_dataframe, write_manifest, load_samplesheet, read_fcs, standardize_channels
from cytoflow_qc.model import Sample, StageResult
from cytoflow_qc.utils import ensure_dir, _read_json, _write_json, timestamp
...
class IngestionStage(PipelineStage):
    ...
    def run(self, input_result: StageResult | None = None) -> StageResult:
        ...
        for row in sheet.to_dict(orient="records"):
            ...
            save_dataframe(events, events_file)
...
class ParallelPipelineStage(PipelineStage):
    """Abstract base class for a parallel pipeline stage."""
    ...
    def run(self, input_result: StageResult) -> StageResult:
        results = []
        ...
        updated_samples, optional_data = zip(*results)
        self.finalize_stage(optional_data)
    ...
    def finalize_stage(self, optional_data: List[Any]) -> None:
        """Optional hook to run after all samples are processed."""
        pass
...
class CompensationStage(ParallelPipelineStage):
    ...
    def process_sample(self, sample: Sample) -> Tuple[Sample, None]:
        events = load_dataframe(sample.events_file)
...
class QCStage(ParallelPipelineStage):
    ...
    def process_sample(self, sample: Sample) -> Tuple[Sample, pd.DataFrame]:
        ...
        qc_df = add_qc_flags(df, self.qc_config.dict())
        
        events_file = self.events_dir / f"{sample.sample_id}.parquet"
        save_dataframe(qc_df, events_file)
        
        sample.events_file = events_file
        return sample, (sample.sample_id, qc_df)

    def finalize_stage(self, optional_data: List[Tuple[str, pd.DataFrame]]) -> None:
        sample_tables = dict(optional_data)
        summary = qc_summary(sample_tables)
        summary.to_csv(self.outdir / "summary.csv", index=False)
        plot_qc_summary(summary, str(self.outdir / "figures" / "qc_pass.png"))
...
class GatingStage(ParallelPipelineStage):
    ...
    def process_sample(self, sample: Sample) -> Tuple[Sample, Dict[str, Any]]:
        ...
        gated, params = auto_gate(df, strategy=self.strategy, config=gate_config)
        
        events_file = self.events_dir / f"{sample.sample_id}.parquet"
        params_file = self.params_dir / f"{sample.sample_id}.json"
        
        save_dataframe(gated, events_file)
        _write_json(params_file, params)
        
        sample.events_file = events_file
        sample.params_file = params_file
        
        summary_row = {"sample_id": sample.sample_id, "input_events": len(df), "gated_events": len(gated)}

        fig = plot_gating_scatter(
            df, gated, gate_config["channels"].get("fsc_a", "FSC-A"),
            gate_config["channels"].get("ssc_a", "SSC-A"),
            str(self.figures_dir / f"{sample.sample_id}_gating.png"),
        )
        plt.close(fig)
...
class DriftStage(PipelineStage):
    ...
    def run(self, input_result: StageResult) -> StageResult:
        ...
        if "condition" in input_result.samples[0].extra_metadata:
