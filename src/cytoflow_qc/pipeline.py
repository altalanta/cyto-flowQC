...
from cytoflow_qc.io import load_dataframe, save_dataframe, load_samplesheet, read_fcs, standardize_channels
...
class PipelineStage(ABC):
    ...
    def run(self, input_result: StageResult | None = None) -> StageResult | None:
        """Execute the pipeline stage, using cache if available."""
    ...
    def _hash_inputs(self, *args) -> str:
        ...
        for arg in args:
            ...
            if isinstance(arg, (str, Path)):
                ...
                hasher.update(arg.json(sort_keys=True).encode())
            else:
                hasher.update(str(arg).encode())
        
        try:
            source = inspect.getsource(self.process_sample)
        except AttributeError:
            source = inspect.getsource(self._run_logic)
...
class IngestionStage(PipelineStage):
    ...
    def _run_logic(self, input_result: StageResult | None = None) -> StageResult:
        ...
        for row in sheet.to_dict(orient="records"):
            ...
            sample = Sample(
                sample_id=sample_id, file_path=row["file_path"], events_file=events_file,
...
class CompensationStage(ParallelPipelineStage):
    ...
    def _get_cache_key(self, input_result: StageResult | None) -> str | None:
        return self._hash_inputs(input_result, self.spill)
...
class QCStage(ParallelPipelineStage):
    ...
    def finalize_stage(self, optional_data: List[Tuple[str, pd.DataFrame]]) -> None:
        ...
        fig = plot_qc_summary(summary, str(self.outdir / "figures" / "qc_pass.png"))
        plt.close(fig)
        
    def _get_cache_key(self, input_result: StageResult | None) -> str | None:
        return self._hash_inputs(input_result, self.qc_config)
